...
nconf.env().file('./src/utils/config-import.json');

// If expected variables are not set, set them to the default locations
nconf.defaults({
  'HORIZONTAL_SLIDER_GOOGLE_CLIENT_SECRETS': '~/.horizontal_slider_google_client_secrets.json',
  'HORIZONTAL_SLIDER_TOKEN': '~/.horizontal_slider_token.json'
});

var secretFile = untildify(nconf.get('HORIZONTAL_SLIDER_GOOGLE_CLIENT_SECRETS'));
...
function primeToken(client, cb) {
  fs.readFile(untildify(nconf.get('HORIZONTAL_SLIDER_TOKEN')), 'utf8', function(err, data) {
...
function buildDataJSON(data) {
  ...
  fs.writeFileSync('./src/data/data.json', JSON.stringify(payload, null, 2));
}
